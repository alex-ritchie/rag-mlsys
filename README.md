...
- Ablation (M8, serving side): single-stream decode is bandwidth-bound — 27B W4A16 (16.6 GiB) and 9B BF16 (16.8 GiB) both 51 tok/s, 9B W4A16 (9.5 GiB) 95 tok/s; peak LLM-only aggregate 593 (27B @c16) vs 968 (9B BF16) vs **1,503 tok/s** (9B W4A16 @c32) vs 674 (35B-A3B MoE, KV-bound); the MoE decodes 3× faster than the 27B per stream (159 vs 51 tok/s) but its resident experts leave 0.6 GiB of KV; llama.cpp with MTP draft-3 lifts the 27B's single stream to 69 tok/s (draft acceptance 56–73 %); end to end (rag-e2e) the 9B W4A16 holds TTFT 1.1 s at c8 with zero reranker OOMs ([ablation-serving.md](docs/benchmarks/ablation-serving.md), [benchmark-report.md](docs/writeups/benchmark-report.md)).
